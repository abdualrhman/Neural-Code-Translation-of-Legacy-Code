...
CONFIG = {
    ...
    "output_dir": "/work3/s204476/Qwen3-32B",
    "num_epochs": 8,
    "train_batch_size": 2,
    "eval_batch_size": 2,
    "warmup_ratio": 0.1,
    ...
    "max_length": 2048,
    "data_dir": "../data/",
    "train_data_file": "../data/BI_train.json",
    "val_data_file": "../data/B_val.json",
    "seed": 42,
...
def build_chat_prompt(tokenizer, apl, description, method_name, method_signatures):
...
Output only the C# program, with no example usage."""
...
def build_full_text(tokenizer, apl, description, csharp, method_name, method_signatures):
    prompt = build_chat_prompt(tokenizer, apl, description, method_name, method_signatures)
...
def tokenize_and_mask_chat(batch, tokenizer, max_length):
    ...
    for i in range(len(batch["apl"])):
        ...
        method_name = batch.get("method_name", [None] * len(batch["apl"]))[i]
        method_signatures = batch.get("method_signatures", [None] * len(batch["apl"]))[i]
        
        full_text, prompt_token_len = build_full_text(
            tokenizer, apl, description, csharp, method_name, method_signatures
        )
...
def load_and_split_dataset(cfg) -> DatasetDict:
    def _load_split(path: str, split_name: str):
        ...
        df['csharp'] = df['csharp'].astype(str).fillna('')
...
def main():
    ...
    steps_per_epoch = 100 // (cfg['train_batch_size'] * cfg['gradient_accumulation_steps'])
    ...
    wandb.init(
        project="apl-to-csharp",
        group="qwen3-32b-chat-template",
