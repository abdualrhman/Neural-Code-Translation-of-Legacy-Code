...
CONFIG = {
    ...
    "test_data_file": "../results/gpt5_apl_to_nl_Bx_test_val.json",
    "data_dir": "../data/",
    "seed": 42,
}
...
def build_prompt(
    ...
    user_content = f"### Natural language summary: {apl_to_nl_pred}\n"
...
def load_test_dataset(cfg) -> Dataset:
    if cfg["test_data_file"] and os.path.exists(cfg["test_data_file"]):
        ...
        missing = [k for k in ["apl", "csharp", "apl_to_nl_pred"] if k not in ds.column_names]
        if missing:
...
def save_results(preds):
    out_file = CONFIG['results_dir'] + "/qwen3_pf_nl_to_cs_Bx_test_val.json"
    test_data = []
...
def main():
    ...
    for datapoint in test_ds:
        apl = datapoint["apl"]
        apl_to_nl_pred = datapoint["apl_to_nl_pred"]
        method_signatures = datapoint["method_signatures"]
        prompts.append(build_prompt(tokenizer, apl, apl_to_nl_pred, method_signatures))
    ...
    preds = [extract_csharp(o) for o in raw_outputs]
    save_results(preds)
