...
CONFIG = {
    ...
    "output_dir": "/work3/s204476/Qwen3-32B-apl-to-nl",
    "num_epochs": 4,
    "train_batch_size": 2,
    "eval_batch_size": 2,
    "warmup_ratio": 0.1,
    ...
    "test_data_file": "../data/AB_test.json",
    "seed": 42,
...
NL_TAG = "### Natural Language Summary:\n"


def build_chat_prompt(tokenizer, apl):
    system_prompt = """You are a careful natural language summarization assistant.
Given the following APL code make a natural language summary of the code.
Output only the natural language summary, with no example usage."""
...
def build_full_text(tokenizer, apl, nl_description):
    prompt = build_chat_prompt(tokenizer, apl)
    full_text = prompt + nl_description + tokenizer.eos_token
...
def tokenize_and_mask_chat(batch, tokenizer, max_length):
    ...
    for i in range(len(batch["apl"])):
        ...
        full_text, prompt_token_len = build_full_text(tokenizer, apl, nl_description)
...
class StopOnStrings(StoppingCriteria):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        for stop in self.stop_ids:
            if len(stop) <= len(input_ids[0]) and input_ids[0].tolist()[-len(stop):] == stop:
                return True
        return False


BAD_WORDS = ["### Explanation", "### Request", "```", "code block"]


def generate_code(model, tokenizer, apl_code, max_new_tokens=256):
    prompt = build_chat_prompt(tokenizer, apl_code)
    inputs = tokenizer(prompt, return_tensors="pt").to(model.device)

    stopping = StoppingCriteriaList([StopOnStrings(STOP_STRS, tokenizer)])
